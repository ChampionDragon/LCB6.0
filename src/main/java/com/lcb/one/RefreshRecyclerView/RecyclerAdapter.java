...
/**
 * Description:RecyclerView的底层适配器
...
public abstract class RecyclerAdapter<T> extends RecyclerView.Adapter<BaseViewHolder<T>> implements IHandler {
    ...
    public void onBindViewHolder(BaseViewHolder<T> holder, int position) {
        Logs.v(TAG + "134:viewCount : " + mViewCount + " position : " + position);
    ...
    public void addAll(List<T> data) {
        ...
        if (!mIsNoMoring && size > 0) {
            ...
            Logs.v(TAG + "323:addAll()  startPosition : " + positionStart + "  itemCount : " + size);
        }
    }
    ...
    public void remove(T object) {
        if (object != null && !mData.contains(object)) {
            Logs.e(TAG + "350:without the object : " + object.getClass().getName());
            return;
        }
...
//                throw new IllegalArgumentException("itemPosition is greater than data size");////上线后这个注释防止程序杀死
                Logs.e(TAG + "377:  删除越界了");
            } else {
