...
/**
 * Description:自定义Tablelayout的TableColumn
...
public class TableColumn extends LinearLayout {
    ...
    private void initContent() {
        ...
        for (String text : content) {
            ...
            maxTextViewWidth = Math.max(maxTextViewWidth, SmallUtil.measureTextViewWidth(textView, text));
            textView.setGravity(getTextGravity(callback.getTableLayout().getTableTextGravity()));
            textView.setPadding(padding, 0, padding, 0);
            textView.setText(text);
            textViews.add(textView);
        }
        LinearLayout.LayoutParams layoutParams = new LayoutParams((int) (padding * 2 + maxTextViewWidth), callback.getTableLayout().getTableRowHeight());
